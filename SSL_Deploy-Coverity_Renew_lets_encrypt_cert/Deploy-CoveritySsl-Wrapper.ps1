...
$logFile = "D:\mydotfile\SSL_Deploy-Coverity_Renew_lets_encrypt_cert\logs\deploy_$timestamp.log"
...
try {
    Set-Location "D:\mydotfile\SSL_Deploy-Coverity_Renew_lets_encrypt_cert"
    ...
    if ($password) {
        ...
        $output = & "D:\mydotfile\SSL_Deploy-Coverity_Renew_lets_encrypt_cert\Deploy-CoveritySsl.ps1" -SambaPassword $password 2>&1
    } else {
